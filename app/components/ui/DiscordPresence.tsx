...
export default function DiscordPresence() {
  ...
  return (
    <div className="w-full relative group">
      ...
      <div className="absolute -inset-4 bg-violet-500/10 blur-2xl rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none -z-10"></div>

      {/* Outer Wrapper buat efek Animated Border */}
      <div className="relative rounded-[2rem] p-[1px] transition-transform duration-500 hover:-translate-y-1 shadow-lg hover:shadow-[0_15px_40px_-15px_rgba(139,92,246,0.3)] overflow-hidden">
        ...
        {/* The Magic: Spinning Gradient Border (Cuma muncul pas di-hover) */}
        <div className="absolute top-1/2 left-1/2 w-[200%] h-[200%] -translate-x-1/2 -translate-y-1/2 bg-[conic-gradient(from_90deg_at_50%_50%,transparent_0%,rgba(139,92,246,0.2)_30%,rgba(139,92,246,0.8)_50%,rgba(16,185,129,0.5)_70%,transparent_100%)] animate-[spin_4s_linear_infinite] opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-0"></div>
        ...
        <div className="relative bg-card/80 backdrop-blur-3xl rounded-[calc(2rem-1px)] p-5 z-10 flex flex-col gap-4">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div className="relative shrink-0">
                ...
                <img 
                  ...
                  className="relative w-11 h-11 rounded-full object-cover ring-2 ring-background z-10" 
                  alt="avatar" 
                />
              </div>
            ...
            {/* Discord Logo (Watermark style) */}
            <div className="bg-white/5 dark:bg-white/10 p-2 rounded-xl border border-white/5">
              <svg className="w-5 h-5 text-muted-foreground/60 group-hover:text-violet-500 transition-colors" viewBox="0 0 24 24" fill="currentColor">
          ...
          <div className="bg-background/50 border border-border/40 rounded-2xl p-3 relative overflow-hidden group/activity transition-colors hover:bg-background/80">
            {active ? (
              <div className="flex items-center gap-3 relative z-10">
                ...
                <div className="relative w-12 h-12 rounded-xl overflow-hidden shrink-0 shadow-md">
                  <img 
                    ...
                    alt="activity"
                    className={`w-full h-full object-cover transition-transform duration-700 ${active.name === 'Spotify' ? 'group-hover/activity:scale-110' : ''}`}
                  />
                  {/* Overlay icon spotify kecil di pojok gambar */}
                  {active.name === 'Spotify' && (
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-center">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    {/* LIVE EQ VISUALIZER (Kalo Spotify) */}
                    {active.name === 'Spotify' && (
                      <div className="flex items-end gap-[2px] h-2.5 opacity-80">
                        <span className="w-[3px] bg-violet-500 rounded-sm animate-[eq_1s_ease-in-out_infinite]"></span>
                        <span className="w-[3px] bg-violet-500 rounded-sm animate-[eq_1.2s_ease-in-out_infinite_0.2s]"></span>
                        <span className="w-[3px] bg-violet-500 rounded-sm animate-[eq_0.8s_ease-in-out_infinite_0.4s]"></span>
                      </div>
                    )}
                  </div>
                ...
                {/* Timer Pill */}
                {timer && (
                  <div className="text-[10px] font-mono font-bold bg-muted/80 border border-border text-foreground px-2 py-1 rounded-md shadow-sm shrink-0">
                    {timer}
                  </div>
                )}
              </div>
